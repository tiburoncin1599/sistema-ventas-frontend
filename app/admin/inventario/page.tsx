                ...
                {pedidos.slice(0, 10).map(p => {
                  ...
                  return (
                    <tr key={p.id} className="border-t dark:border-gray-700 dark:text-white">
                      ...
                      <td className="px-6 py-4 text-sm font-medium">${Number(p.total).toFixed(2)}</td>
                      <td className="px-6 py-4">
                      ...
                      >
                        ...
                        <div className="flex items-end justify-between">
                          ...
                          <div className="text-right">
                            <p className="font-bold text-gray-900 dark:text-white">${Number(item.producto?.precio).toFixed(2)}</p>
                            {item.producto?.precio_por_docena && (
                              <p className="text-xs text-gray-500 dark:text-gray-400">Docena: ${Number(item.producto.precio_por_docena).toFixed(2)}</p>
                            )}
                          </div>
                        </div>
                        <div className="mt-3 pt-3 border-t dark:border-gray-700 flex justify-between items-center">
                          ...
                          <span className="text-xs text-gray-400 dark:text-gray-500">
                            ${(item.cantidad * Number(item.producto?.precio)).toFixed(2)} en stock
                          </span>
                        </div>
                      </div>
                    );
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/30 rounded-xl p-4">
                <div className="flex justify-between items-center py-1">
                  ...
                  <span className="font-bold dark:text-white">${Number(modalItem.producto?.precio).toFixed(2)}</span>
                </div>
                {modalItem.producto?.precio_costo && (
                  <div className="flex justify-between items-center py-1">
                    <span className="text-sm text-gray-600 dark:text-gray-400">Precio costo</span>
                    <span className="font-medium dark:text-white">${Number(modalItem.producto.precio_costo).toFixed(2)}</span>
                  </div>
                )}
                {modalItem.producto?.precio_por_docena && (
                  <div className="flex justify-between items-center py-1">
                    <span className="text-sm text-gray-600 dark:text-gray-400">Precio por docena</span>
                    <span className="font-medium dark:text-white">${Number(modalItem.producto.precio_por_docena).toFixed(2)}</span>
                  </div>
                )}
                <div className="flex justify-between items-center py-1 border-t dark:border-gray-600 mt-1 pt-2">
                  ...
                  <span className="font-bold text-blue-600 dark:text-blue-400">${(modalItem.cantidad * Number(modalItem.producto?.precio)).toFixed(2)}</span>
                </div>
              </div>
