      ...
      {resumen && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-orange-50 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-800 rounded-2xl p-5">
            ...
            <p className="text-3xl font-bold text-orange-700 dark:text-orange-300">${Number(resumen.total_pendiente).toFixed(2)}</p>
          </div>
          <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-2xl p-5">
            ...
            <p className="text-3xl font-bold text-green-700 dark:text-green-300">${Number(resumen.total_pagado).toFixed(2)}</p>
          </div>
        </div>
      )}
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {grupo.deudas.map(d => (
                          <tr key={d.id} className="border-t dark:border-gray-700 dark:text-white">
                            ...
                            <td className="px-6 py-4 text-sm">${Number(d.monto).toFixed(2)}</td>
                            <td className="px-6 py-4 text-sm">${Number(d.monto_pagado).toFixed(2)}</td>
                            <td className="px-6 py-4 font-bold text-sm">${saldoPendiente(d).toFixed(2)}</td>
                      ...
                      <tfoot>
                        <tr className="border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                          ...
                          <td className="px-6 py-3 font-bold text-sm dark:text-white">${totalDeuda.toFixed(2)}</td>
                          <td className="px-6 py-3 font-bold text-sm dark:text-white">${totalPagado.toFixed(2)}</td>
                          <td className="px-6 py-3 font-bold text-sm dark:text-white">${saldo.toFixed(2)}</td>
                          <td colSpan={2} />
                        </tr>
                      </tfoot>
                    </table>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 max-w-md w-full mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between">
                ...
                <span className="font-bold">${Number(showPagar.monto).toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Ya pagado:</span>
                <span className="font-medium dark:text-white">${Number(showPagar.monto_pagado).toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-lg border-t dark:border-gray-700 pt-3">
                ...
                <span className="font-bold text-orange-600 dark:text-orange-400">${saldoPendiente(showPagar).toFixed(2)}</span>
              </div>
            </div>
            <div className="mb-6">
