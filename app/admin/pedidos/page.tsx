              ...
              {pedidoSel.detalles?.map(d => (
                <div key={d.id} className="flex items-center gap-4 border dark:border-gray-700 rounded-xl p-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-sm">x{d.cantidad} @ ${Number(d.precio_unitario).toFixed(2)}</p>
                  </div>
                  <p className="font-bold dark:text-white">${(d.cantidad * Number(d.precio_unitario)).toFixed(2)}</p>
                </div>
              ))}
            </div>

            <p className="text-2xl font-bold text-blue-600 dark:text-blue-400 mb-6">Total: ${Number(pedidoSel.total).toFixed(2)}</p>
